...
export default function Page() {
  ...
  const circularKeyframes = {
    x: [isMobileView ? -50 : -100, isMobileView ? 1000 : 1800], // Circular motion
  ...
  return (
    <div className='flex flex-col items-center justify-center px-4 md:px-8 lg:px-16 '>
        {!animationComplete && (
          <motion.div
            ...
            animate={{
              ...
              scale: [2, 2.5, 3, 4, 5, 5.5, 6],
              rotate: [0, -10, -20], // Fade in and fade out
            }}
            transition={{
      ...
      <h1 className='text-3xl md:text-4xl lg:text-6xl font-semibold text-center pt-3 text-black/90'>TIMELINE</h1>
      <div className='flex flex-col gap-8 md:gap-16 w-full'>
        <Timeline data={timelineData} />
        <div className='space-y-16'>
          <div className='flex flex-col gap-10 items-center'>
            <h1 className='text-3xl md:text-4xl lg:text-6xl font-semibold text-center text-black/90'>CHAPTERS EVENTS</h1>
            <ChaptersCard testimonials={Chapters} />
          </div>
          <div className='flex flex-col gap-4 items-center'>
            <h1 className='text-3xl md:text-4xl lg:text-6xl font-semibold text-center text-black/90'>SPONSORS</h1>
            <Sponsores />
          </div>
        </div>
      </div>
